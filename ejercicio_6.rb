...
#1
arr_genero = movies.map {|k,v| v[:genre].downcase}.uniq
puts arr_genero
puts ""


#2
arr_genero = movies.map {|k,v| v[:genre].downcase} #genera arreglo con todos lo géneros de película
#genera un hash que agrupa los géneros de película: {"scyfy"=>["scyfy", "scyfy", "scyfy"], "horror"=>["horror"]}
hash_genero = arr_genero.group_by {|x| x} 
#cuenta e imprime los genéros de película
hash_genero.map {|k,v| puts "#{k}: #{v.count}"}
...
#4
hash_peli_votes = {}
hash_peli_votes_final = {}

#genera el hash hash_peli_votes, de la forma:
#"Consuelo Fields"=>{:bad=>[:bad, :bad, :bad, :bad, :bad, :bad, :bad, :bad], :good=>[:good, :good, :good, :good]}}
movies.map {|k,v| hash_peli_votes[k] = v[:votes].group_by{|x| x}}

#k es el nombre de la pelicula; v es un hash con los votos agrupados
hash_peli_votes.map do |k,v| 
  hash_votes_cont = {}
  #se recorre el hash de los votos
  #key es el voto y val es el arreglo con el detalle de los votos
  v.each do |key,val|
    hash_votes_cont[key] = val.count
  end

  hash_peli_votes_final[k] = hash_votes_cont
end

#recorre hash_peli_votes_final para determinar si la película es buena, mala, regular
#k es el nombre de la película y v es un hash con el total de votos buenos y malos
hash_peli_votes_final.map do |k,v|
  if v[:bad] > v[:good]
    puts "Ranking #{k}: Mala (Mala:#{v[:bad]}, Buena:#{v[:good]})"
  elsif v[:bad] < v[:good]
    puts "Ranking #{k}: Buena (Mala:#{v[:bad]}, Buena:#{v[:good]})"
  else
    puts "Ranking #{k}: Regular (Mala:#{v[:bad]}, Buena:#{v[:good]})"
  end
end    
